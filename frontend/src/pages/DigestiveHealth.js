...
const DigestiveHealth = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        const params = {
          health_benefit_id: 17, // Digestive Health ID
          page,
          per_page: 20,
        };
          ...
          <div className="w-full flex flex-col md:flex-row items-start md:items-center justify-between">
            <div className="flex-1 mb-4 md:mb-0">
              ...
              <div className="text-sm md:text-base mb-2" style={{ color: '#000' }}>
                <Link to="/" className="hover:text-green-600 transition-colors" style={{ color: '#000' }}>HOME</Link> &gt; DIGESTIVE HEALTH
              </div>
            </div>
          </div>
        </div>
          ...
          <main className="lg:w-3/4 order-2 lg:order-1">
            {loading ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-600 text-lg">No products found for Digestive Health</p>
              </div>
            ) : (
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-6">
